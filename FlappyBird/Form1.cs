...
namespace FlappyBird
{
    public partial class Form1 : Form
    {
        ...
        float gravity;
        public Form1() // начальная инициализация окна
        {
            InitializeComponent();
            timer1.Interval = 10;
        ...
        public void Init() // создание начальных объектов при запуске
        {
        ...
        private void update(object sender, EventArgs e)  // функция привзанная к тикам таймера
        {
            if (bird.y > 600) // проверка по высоте
            {
                bird.isAlive = false;
                timer1.Stop();
                Init();
            ...
            {
                bird.isAlive = false; // рестарт после столкновения
                timer1.Stop();
                Init();
            }

            if (bird.gravityValue != 0.10f) // ускорение падения
                bird.gravityValue += 0.0046f;
            gravity += bird.gravityValue;
            bird.y += gravity;

            if (bird.isAlive) { // вызов функции перемещения сиен
                MoveWalls();
            }
        ...
        private bool Collide(Player bird, TheWall wall1) // проверка на столкновение
        {
        ...
        private void CreateNewWall() // перемещение объектов верхней и нижней трубы
        {
        ...
        private void OnPaint(object sender, PaintEventArgs e) // отрисовка на форме
        {
        ...
        private void button1_Click(object sender, EventArgs e) // изменение гравитации по нажатию
        {
            if (bird.isAlive)
            {
                gravity = -1;
